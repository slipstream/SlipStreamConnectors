...
(defn get-lifetime
  [x509cred]
  (let [cert (.getCertificate x509cred)
        not-after (.getTime (.getNotAfter cert))
        not-before (.getTime (.getNotBefore cert))]
    (int (/ (- not-after not-before) 1000.))))

(defn voms-request
  [vo fqans targets lifetime]
...
(defn add-acs
  ...
  [cred acs]
  (let [lifetime (get-lifetime cred)
        proxy-cert-options (-> (.getCertificateChain cred)
                               (ProxyCertificateOptions.))]
    (.setLifetime proxy-cert-options lifetime)
...
(defn write-proxy
  ...
  [os proxy]
  (try
    (CredentialsUtils/saveProxyCredentials os proxy)
    (catch Exception e
      (CredentialsUtils/saveProxyCredentials os (.getCredential proxy)))))
