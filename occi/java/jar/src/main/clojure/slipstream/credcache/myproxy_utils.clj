...
(defn get-params
  ...
  [{:keys [username passphrase lifetime want-trustroots cred-name proxy]
    :or   {lifetime        ttl-12h
           want-trustroots false}}]
  (let [getparams
        (doto (GetParams.)
          (.setUserName (or username (.toString (.getName proxy))))
...
(defn get-proxy
  "Gets the GSS credental from the MyProxy server.  All of the parameters
   are passed as a map with the following keys: :myproxy-host, :myproxy-port,
...
(defn put-proxy
  "Stores a credential on a myproxy server.  The argument is a map
   ...
   credential."
  [{:keys [myproxy-host myproxy-port proxy lifetime]
    :or   {myproxy-host default-myproxy-host
           myproxy-port default-myproxy-port
           lifetime     ttl-12h}
    :as   params}]
...
(defn destroy-params
  ...
  [{:keys [username passphrase cred-name proxy]}]
  (let [destroyparams
        (doto (DestroyParams.)
          (.setUserName (or username (.toString (.getName proxy))))
          (.setCredentialName cred-name))]
