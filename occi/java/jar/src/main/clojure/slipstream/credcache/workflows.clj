...
(defn renew-gsscred
  ...
  (let [cred-info (db/retrieve-resource id)
        ...
        params (assoc cred-info :proxy proxy)
...
;;
;; (def delegated-proxy (myproxy/get-proxy {:proxy bootstrap-proxy}))
;; (def vproxy (voms/gsscred->vproxy delegated-proxy {"vo.lal.in2p3.fr" {}}))
;; (voms/write-proxy "vproxy.pem" vproxy)
;; voms-proxy-info --chain --file vproxy.pem  ;; show complete chain of generated proxy
...
(defn full-workflow
  ...
  (let [delegation-params (upload-delegation-proxy passphrase)
        ...
        _ (myproxy/destroy-proxy (assoc bootstrap-params :proxy bootstrap-proxy))
        delegated-proxy (myproxy/get-proxy {:proxy bootstrap-proxy})
        vproxy (voms/gsscred->vproxy delegated-proxy {})]
    ...
    (assert ((complement nil?) vproxy) "voms proxy creation failed")))
...
(defn renew-delegation
  ...
  (let [cred-info (db/retrieve-resource id)
        ...
        gsscred (myproxy/get-proxy (assoc cred-info :proxy gsscred))
